...
class PublicChrome
{
    ...
    private function navigation(): array
    {
        ...
        return $this->cached(PublicChromeCache::PRIMARY,function() use($fallback){if(!Schema::hasTable('navigation_menus'))return $fallback;$menu=NavigationMenu::query()->where('location','primary')->where('is_active',true)->with(['items'=>fn($q)=>$q->with(['page','solution','product'])->where('is_active',true)->orderBy('display_order')->orderBy('id')->limit(100)])->first();if(!$menu)return $fallback;$items=$menu->items->groupBy('parent_id'); return $this->tree($items,null,1,3);},$fallback,1);
    }
    private function tree($groups, ?int $parent, int $depth, int $max): array
    {
        if($depth>$max)return []; return $groups->get($parent,collect())->take($parent===null?12:20)->map(function($item) use($groups,$depth,$max){$pageVisible=$item->link_type==='page'&&$item->page&&$item->page->isPubliclyVisible();$solutionVisible=$item->link_type==='solution'&&$item->solution&&$item->solution->isPubliclyVisible();$productVisible=$item->link_type==='product'&&$item->product&&$item->product->isPubliclyVisible();$href=$pageVisible?route('pages.show',$item->page->slug):($solutionVisible?route('solutions.show',$item->solution->slug):($productVisible?route('products.show',$item->product->slug):PublicLink::href($item->link_type,$item->route_name,$item->url)));return ['id'=>'nav-'.$item->id,'label'=>$item->label,'href'=>$href,'target'=>$item->target,'external'=>PublicLink::isExternal($item->url),'route'=>$item->route_name,'disabled'=>$item->link_type==='disabled'||(in_array($item->link_type,['page','solution','product'],true)&&!$pageVisible&&!$solutionVisible&&!$productVisible),'featured'=>(bool)$item->is_featured,'mega'=>(bool)$item->opens_mega_menu,'description'=>$item->description,'badge'=>$item->badge_text,'children'=>$this->tree($groups,$item->id,$depth+1,$max)];})->values()->all();
    }
