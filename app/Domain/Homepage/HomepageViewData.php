...
final class HomepageViewData
{
    ...
    private function load(bool $preview): ?array
    {
        ...
        $sections = HomepageSection::query()
            ...
            ->with(['items' => fn ($query) => $query->where('is_active', true)->orderBy('display_order')->orderBy('id')->limit(24)])
            ...
            ->map(function (HomepageSection $section) use ($registry): array {
                ...
                return [
                    ...
                    'items' => $section->items->where('item_type', $definition['item_type'])->take($definition['maximum'])->map(fn ($item): array => [
                        'id' => $item->id, 'title' => $item->title, 'eyebrow' => $item->eyebrow, 'description' => $item->description,
                        'secondary_text' => $item->secondary_text, 'highlighted_text' => $item->highlighted_text, 'icon' => $item->icon,
                        'badge' => $item->badge, 'image_path' => PublicAssetPath::isSafe($item->image_path) ? $item->image_path : null, 'mobile_image_path' => PublicAssetPath::isSafe($item->mobile_image_path) ? $item->mobile_image_path : null,
