...
new class extends Component {
    ...
    public function testConnection(DatabaseConnectionTester $tester, InstallationState $state): void
    {
...
<x-layouts.installer :current-step="4">
    ...
    <div class="flex flex-col gap-7">
        <div><p class="text-sm font-semibold text-blue-700">Step 4 of 4</p><h1 class="mt-2 text-2xl font-bold tracking-tight sm:text-3xl">Connect your database</h1><p class="mt-3 leading-6 text-slate-600">We will perform a harmless connection test only. No tables, migrations, or data will be changed.</p></div>
        ...
        <form wire:submit="testConnection" class="grid gap-5 sm:grid-cols-2" autocomplete="off">
            ...
            <div class="flex flex-col gap-3 border-t border-slate-200 pt-6 sm:col-span-2 sm:flex-row sm:items-center sm:justify-between"><p class="text-xs leading-5 text-slate-500">Credentials are used only for this test and are not saved.</p><flux:button type="submit" variant="primary" wire:loading.attr="disabled" class="min-h-11 w-full justify-center sm:w-auto" icon="circle-stack">Test Database Connection</flux:button></div>
        </form>
    </div>
</x-layouts.installer>
