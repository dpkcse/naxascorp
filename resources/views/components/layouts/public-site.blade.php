@props(['settings' => null, 'siteName' => 'Naxora CMS', 'legalName' => 'Naxas Innovations Limited', 'navigation' => [], 'chrome' => [], 'pageTitle' => 'Naxora — Premium Corporate Website CMS', 'description' => 'A premium corporate website foundation.', 'canonical' => null, 'robots' => 'index, follow', 'image' => null])
<!DOCTYPE html>
...
<head>
    ...
    @if(data_get($chrome, 'branding.favicon'))<link rel="icon" href="{{ asset(data_get($chrome, 'branding.favicon')) }}">@endif
...
<x-public.top-bar :settings="$settings" :header="data_get($chrome, 'header', [])" /><x-public.header :site-name="$siteName" :navigation="$navigation" :branding="data_get($chrome, 'branding', [])" :header-settings="data_get($chrome, 'header', [])" />
<main id="main-content" tabindex="-1">{{ $slot }}</main>
<x-public.footer :settings="$settings" :site-name="$siteName" :legal-name="$legalName" :branding="data_get($chrome, 'branding', [])" :footer="data_get($chrome, 'footer', [])" />
